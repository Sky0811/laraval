...
<script type="text/javascript" async="" src="{{asset('home/evaluate/js/unjcV2.js')}}"></script><script type="text/javascript" async="" src="{{asset('home/evaluate/js/mstr.js')}}"></script><script type="text/javascript" async="" src="{{asset('home/evaluate/js/jquery.js')}}"></script><script type="text/javascript" async="" src="{{asset('home/evaluate/js/xmst.js')}}"></script>
...
<script>
Comment2017 = {
    commentDomain: 'http://comment.huodong.mi.com',
    comment_proId: 0,
    comment_virtualId: 10000022}
</script>
<div id="J_proHeader"><div class="xm-product-box"> <div class="nav-bar" id="J_headNav"> <div class="container J_navSwitch"> <h2 class="J_proName">{{ $good->Goods_name }}</h2> <div class="con"><div class="right"><a href="javascript:void(0);" class="cur" data-stat-id="63093bcebf4ac2f6" onclick="_msq.push(['trackEvent', 'fbdf2c86fa0fa944-63093bcebf4ac2f6', 'javascript:void0', 'pcpid', '']);">用户评价</a></div></div> </div> </div></div></div>
<div class="m-comment-wrap h-comment-wrap">
    <div class="container J_commentWrap"><div class="row"> <div class="span13 h-comment-main  m-comment-main J_commentCon" style="width:100%;"> 
    <!-- <div style='width:100%;height:100px;background:white;' class="state">
    <input type="submit" value="好评" style="width:150px;height:40px;background:#F5F5F5;margin:40px 0 0 50px;border:0px;color:#B0B6C7"  class="state2">
    <input type="submit" value="差评" style="width:150px;height:40px;background:#F5F5F5;margin:40px 0 0 50px;border:0px;color:#B0B6C7;"  class="state3"><span style="display:none;">{{ $good->Goods_id }}</span>
    </div> -->
    ...
    <div class="comment-top"> <h2 class="m-tit">评价</h2></div> 
...
<script src="{{asset('home/evaluate/js/base.js')}}"></script>
    ...
    <!-- <script type="text/javascript">
    $('.state').children().click(function(){
        var gid = $('.state3').siblings(0).text();
        if($(this).val()=='好评'){
            var state =1;
        }else{
            var state =2;
        ...
        $.ajax({
            url:'/evaluat',
            type:'get',
            data:{Eval_gid:gid,Eval_state:state},
            async:true,
            dataType:'json',
            success:function(data){
                // console.log(data);
                for (var i = data.length - 1; i >= 0; i--) {
                    $('#ulid').append('<li class="com-item J_resetImgCon J_canZoomBox" data-id="143276777"><div class="comment-info"><a class="user-name" href="">'+data[i].Eval_name+'</a><p class="time">星期四</p> </div> <div class="comment-txt"> <a href="" target="_blank">'+data[i].Eval_text+'</a> </div>  <div class="m-img-list clearfix h-img-list">      <div class="img-item img-item1 item-one showimg"><div class="loader loader-gray"></div> </div>  <div class="J_zoomImgList" style="display: none;">  <span data-src="//i1.mifile.cn/a2/1492681706_7513463_s1080_2040wh.jpg"></span>  </div> </div><div class="comment-answer">  <div class="answer-item"><div class="answer-content"> <h3 class="official-name">官方回复</h3> <p> '+data[i].Eval_reply+'</p> </div> </div>    </div> </li>   ');
                }
